...
const useSourceMaps = false; // 🔥 Desactivar completamente los Source Maps
...
export default {
    ...
    output: [
        ...
        },
        {
            file: 'dist/plugin.js',
            format: 'iife',
            sourcemap: false,  // 🔥 Eliminado por completo
        }
    ],

    onwarn: () => {
        /* Se desactivan los warnings */
    },
    ...
    plugins: [
        typescript({
            sourceMap: false, // 🔥 Asegurar que no genere archivos .map
            inlineSources: false,
        }),
        rollupSwc({
            include: ['**/*.ts', '**/*.svelte'],
            sourceMaps: false, // 🔥 Desactivar Source Maps en SWC
        }),
        rollupSvelte({
            emitCss: false,
            preprocess: {
                style: less({
                    sourceMap: false, // 🔥 Desactivar Source Maps en Svelte
                    math: 'always',
                }),
                script: data => {
                    const preprocessed = sveltePreprocess({ sourceMap: false });
